...
让我困惑，`ARCH=arm make mrproper`, `CONFIG_XXX=m`，这些东西完全不知道是什么意
...
3. 先编译内核。在cubieboard上`zcat /proc/config.gz > .config`，然后将
   ...
   在进行所有的`ARCH=arm make`前，需要在`.config`中设置
...
4. 编译8188eu的驱动，从[这里][8188eu-driver-github]下载驱动的代码。
   ...
   然后：
        ...
        CONFIG_RTL8188EU=m ARCH=arm make -C /path/to/linux-sunxi-3.0 M=`pwd`
...
能加入内核。iptables不能像网卡的驱动那样通过单独编译内核模块的方式来添
加，因为iptables的依赖实在是太多了，如果都编译成内核模块，它们的依赖关
系很难理得清，因此需要重新编译内核，将netfilter的这些功能都编译成
built-in的。具体的操作如下：

1. 编译内核时的配置选项是非常多的，不可能一个一个地选，因此最好的办法
   就是在一个基础上增删功能，因此先用`zcat /proc/config.gz >
   kernel.config`将原内核的配置信息导出来，将它拷到linux-sunxi-3.0的根
   目录下，改名为`.config`，然后`ARCH=arm make menuconfig`，在
   `Networking support -> Networking options -> Network packet
   filtering farmework (Netfilter)`中将**相关的选项**都选成`y`，即以
   built-in的方式编译进内核。这么多选项该怎么选呢？我的做法是，除了标
   为"experimental"的其他的都选上。
   
2. 然后进行`ARCH=arm make uImage`，在`arch/arm/boot`目录下会有一个叫
   `uImage`的文件生成，将它拷出来。
   
3. 在cubieboard中，将boot分区加载进来，我的是：

        mount -t vfat /dev/mmcblk0p1 /media/boot

   然后将新的uImage拷进行，将老的替换。在此之前，你有必要把旧的uImage
   备份一下。（万一新的kernel挂了，可以将备份的uImage再换回去）
4. 重启系统。如果不出意外的话，应该一切OK。重启后用`sudo
   iptables -L`试一下，如果不报错就说明成功了。因为重新编译了内核，原
   来的网卡驱动不好使了，所以还要重复编译网卡的工作，将`8188eu.ko`再编
   译一遍。我做的过程中一切都很顺利，我想，自从看了那篇
